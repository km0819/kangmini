...
# 데이터베이스 실시간 로드 및 로그인 객체 바인딩
db = load_db_users()
auth = stauth.Authenticate(db, "k_secure_cookie", "secure_signature_key_2026", 30)
...
with st.sidebar:
            ...
            with st.form("c_f", clear_on_submit=True):
                ...
                if st.form_submit_button("신규 계정 허가") and nid and nn and np:
                    if nid in db["usernames"]: st.error("⚠️ 이미 가입된 ID")
                        ...
                        supabase.table("users_db").insert({"id": nid, "name": nn, "password": stauth.Hasher.hash(np)}).execute()
                        st.success(f"🎉 계정 생성 완료!"); time.sleep(0.5); st.rerun()
        with tm:
            for u in [k for k in db["usernames"].keys() if k != "admin"]:
                if st.button(f"🗑️ {db['usernames'][u]['name']}({u}) 파기", key=f"d_{u}"):
        ...
        with st.form("p_f", clear_on_submit=True):
            ...
            if st.form_submit_button("암호 변경 적용") and np == cnp and np:
                if bcrypt.checkpw(cp.encode(), db["usernames"][uid]["password"].encode()):
                    supabase.table("users_db").update({"password": stauth.Hasher.hash(np)}).eq("id", uid).execute()
    ...
    for r in rooms:
        ...
        with c2:
            if st.button("❌", key=f"dr_{r['room_id']}", use_container_width=True):
...
# 4. 실시간 메시지 발송 및 실시간 DB 동기화 업서트(Upsert)
