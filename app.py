...
from google.genai import types  # 시스템 지침 설정을 위해 추가
import json
import os
...
# 4. 제미나이 API 클라이언트 초기화 (발급받은 실제 API 키 입력)
...
# 6. 상단 사이드바에 내역 관리 및 크레딧 추가
with st.sidebar:
    st.header("✨ AI 프로필")
    st.markdown("**이름:** kangmini (강미니)")
    ...
    with st.chat_message("kangmini", avatar="✨"):
        with st.spinner("생각 중..."):
            try:
                ...
                # 💡 진짜 제미나이처럼 행동하도록 시스템 지침(System Instruction) 설정
                system_prompt = (
                    "당신의 이름은 'kangmini(강미니)'입니다. 구글의 제미나이(Gemini) 모델을 기반으로 만들어진 "
                    "매우 똑똑하고 친절한 AI 어시스턴트입니다. 사용자가 이름을 물어보면 반드시 'kangmini'라고 답해야 합니다. "
                ...
                response = client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        system_instruction=system_prompt  # 👈 페르소나 주입
                    )
                )
